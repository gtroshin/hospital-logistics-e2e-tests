...
class Main {
	constructor() {
		...
		this.entityItem = (x) =>
			Selector("a.dropdown-item").withAttribute("href", "/" + x);
		this.loginButton = Selector("#login");
		this.usernameInput = Selector("#username");
		this.passwordInput = Selector("#password");
		this.submitLoginFromButton = Selector(
			'button.btn.btn-primary[type="submit"]'
		);
	}

	async openMainMenu() {
		await t.expect(this.accountMenu.visible).ok().click(this.accountMenu);
	}

	async openSignIn() {
		await t.expect(this.loginButton.visible).ok().click(this.loginButton);
	}
