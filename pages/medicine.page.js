...
class Medicine {
    constructor () {
        ...
        this.rowContaining = (medicineName, medicineDescription) => XPathSelector(
            `//tr[./td[contains(., '${medicineName}')] and ./td[contains(., '${medicineDescription}')]]`);
        this.rowAction = ({medicineName, action}) => Selector('td').withText(medicineName).parent('tr').find(`button:has(span[jhitranslate='entity.action.${action}'])`);
    }
    ...
    async verifyMedicine(medicineName, medicineDescription) {
        await scrollDownUntilExists(this.rowContaining(medicineName, medicineDescription));

        await t
            .expect(this.rowContaining(medicineName, medicineDescription).visible).ok()
    }
}
