import { baseUrl, username, password, randomString, getMedicinesByNameAndDescription } from "../utilities/utilities";
...
test.meta('opt', 'true')('I should be able to add a new medicine with all required information and verify it in the system (database)', async t => {
    ...
    await medicinePage.verifyMedicine(t.ctx.medicineName, t.ctx.medicineDescription);
    ...
    await t.expect(numberOfMatchedMedicines).eql(1, 'There should be only one medicine with the provided name and description');
});
